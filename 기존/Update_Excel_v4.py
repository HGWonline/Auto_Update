...
def update_prices(scraped_m_data, itemlist_barcodes, itemlist_sheet):
    for barcode, data in scraped_m_data.items():
        if barcode in itemlist_barcodes:
            row_idx = itemlist_barcodes[barcode]
            # retailPrice를 K열과 L열에 업데이트
            itemlist_sheet.cell(row=row_idx + 1, column=11).value = data['retail_price']  # K열
            itemlist_sheet.cell(row=row_idx + 1, column=12).value = data['retail_price']  # L열
            # wholesalerPrice를 O열에 업데이트
            itemlist_sheet.cell(row=row_idx + 1, column=15).value = data['wholesaler_price']  # O열
    ...
    for barcode, data in scraped_data.items():
        if barcode in itemlist_barcodes:
            ...
            if 'br_discount' in data and data['br_discount'] is not None:
                itemlist_sheet.cell(row=row_idx + 1, column=11).value = data['br_discount']  # K열
...
update_prices(scraped_m_data, itemlist_barcodes, itemlist_sheet)
