...
# 4. 순서를 맞추는 함수 (매번 최신 위치 확인)
def reorder_barcodes(scraped_data, itemlist_sheet):
    """
    Scraped 파일의 바코드 순서에 맞게 Itemlist 파일의 바코드를 재배열.
    바코드를 하나씩 옮긴 후, 최신 위치 정보를 다시 가져와서 순서를 확인.
    """
    insert_position = 2  # 2행부터 시작
    
    # Scraped 데이터의 순서대로 진행
    for scraped_barcode in scraped_data.keys():
        scraped_barcode = str(scraped_barcode).strip()
        
        # 매번 최신의 Itemlist 바코드 정보를 가져옴
        itemlist_barcodes = get_itemlist_barcodes(itemlist_sheet)
        
        if scraped_barcode in itemlist_barcodes:
            current_row = itemlist_barcodes[scraped_barcode]  # 현재 해당 바코드의 위치
            
            # 올바른 위치에 있지 않다면 이동
            if current_row != insert_position:
                print(f"Moving barcode {scraped_barcode} from row {current_row} to {insert_position}")
                row_data = [cell.value for cell in itemlist_sheet[current_row]]
                
                # 현재 위치에서 행 삭제 후, 원하는 위치에 삽입
                itemlist_sheet.delete_rows(current_row)
                itemlist_sheet.insert_rows(insert_position)
                
                # 삭제된 행의 데이터를 새로운 위치에 다시 입력
                for col_idx, value in enumerate(row_data, start=1):
                    itemlist_sheet.cell(row=insert_position, column=col_idx, value=value)
                
                # 이동 후, 모든 바코드 위치 정보가 변경되므로 최신화 필요
                itemlist_barcodes = get_itemlist_barcodes(itemlist_sheet)
                
            # 다음 삽입 위치로 이동
            insert_position += 1

# Itemlist 파일 로드 후 중복 바코드 검사
check_duplicate_barcodes(itemlist_sheet)

# 1단계: 바코드를 비교하여 추가 및 삭제할 바코드 확인
itemlist_barcodes = get_itemlist_barcodes(itemlist_sheet)
...
# 3단계: 추가할 바코드를 가장 아래에 추가 + 바코드 중복 검사
...
# Description, Booragoon Stock, Discount Price 정보를 각각 Title, Variant Inventory Qty, Variant Price로 매칭
if itemlist_sheet:
